...
#define DECODE_CTRL_ROW_POS  10
#define DECODE_CTRL_BANK_POS 13
#define DECODE_CTRL_CID_POS  16
#define DECODE_CTRL_RANK_POS 19

/* DMC-Bing register specific definitions */
...
static int ddr_get_mem_size(uint64_t *size)
{
    ...
    *size = (uint64_t)size_gb * 2ULL * FWK_GIB;
#endif
    ...
    struct dimm_info *info)
{
    ...
    for (i = 1; i <= ddr_info.number_of_ranks; i++) {
        ...
        if (dmc->DIRECT_ADDR != DDR_ADDR_TRAIN_TYPE_WR_LVL) {
            for (j = 1; j <= ddr_info.number_of_ranks; j++) {
                morello_wrlvl_phy_obs_regs(ddr_id, j, info);
            }
            return FWK_E_DEVICE;
        }
        ...
        morello_wrlvl_phy_obs_regs(ddr_id, i, info);
    }
    morello_verify_phy_status(ddr_id, DDR_ADDR_TRAIN_TYPE_WR_LVL, info);
    ...
    dmc->DIRECT_ADDR =
        (DDR_ADDR_TRAIN_TYPE_RD_GATE | (0x3FFFF << DDR_ADDR_DATA_SLICES_POS));
    dmc->DIRECT_CMD = ((ddr_info.ranks_to_train << 16) | 0x000A);

    status = ddr_poll_training_status(dmc);
    if (status != FWK_SUCCESS) {
        for (j = 1; j <= ddr_info.number_of_ranks; j++) {
            morello_read_gate_phy_obs_regs(ddr_id, j, info);
        }
        return status;
    }
...
#    endif
    ...
    /* Run training on slices 0-9 */
    dmc->DIRECT_ADDR =
        (DDR_ADDR_TRAIN_TYPE_RD_EYE | (0x300FF << DDR_ADDR_DATA_SLICES_POS));
    dmc->DIRECT_CMD = ((ddr_info.ranks_to_train << 16) | 0x000A);

    status = ddr_poll_training_status(dmc);
    if (status != FWK_SUCCESS) {
        for (j = 1; j <= ddr_info.number_of_ranks; j++) {
            morello_phy_obs_regs(ddr_id, j, info);
        }
        return status;
    }
    ...
    /* Run training on slices 10-17 */
    dmc->DIRECT_ADDR =
        (DDR_ADDR_TRAIN_TYPE_RD_EYE | (0xFF00 << DDR_ADDR_DATA_SLICES_POS));
    dmc->DIRECT_CMD = ((ddr_info.ranks_to_train << 16) | 0x000A);
...
static int dmc_bing_verify_phy_status(fwk_id_t ddr_id)
{
    int status;

    status = morello_verify_phy_status(
        ddr_id, DDR_ADDR_TRAIN_TYPE_WR_LVL, &ddr_info);
    if (status != FWK_SUCCESS) {
        return status;
    }

    status = morello_verify_phy_status(
        ddr_id, DDR_ADDR_TRAIN_TYPE_RD_GATE, &ddr_info);
    if (status != FWK_SUCCESS) {
        return status;
    }
    ...
    status =
        morello_verify_phy_status(ddr_id, DDR_ADDR_TRAIN_TYPE_VREF, &ddr_info);
    if (status != FWK_SUCCESS) {
        return status;
    }
...
static int direct_ddr_cmd(struct mod_dmc_bing_reg *dmc)
{
    ...
    execute_ddr_cmd(dmc, 0x00000004, 0x0001000A, 0);
    execute_ddr_cmd(
        dmc, 0x00000006, ((ddr_info.ranks_to_train << 16) | 0x0004), 0);
    execute_ddr_cmd(dmc, 0x00000000, 0x0001000B, 0);
    execute_ddr_cmd(
        dmc, 0x00000001, ((ddr_info.ranks_to_train << 16) | 0x000B), 0);
    execute_ddr_cmd(dmc, 0x000003E8, 0x0001000D, 0);
    execute_ddr_cmd(dmc, 0x00000258, 0x0001000D, 1);
    execute_ddr_cmd(
        dmc, 0x00010001, ((ddr_info.ranks_to_train << 16) | 0x000B), 0);
    execute_ddr_cmd(dmc, 0x0000002A, 0x0001000D, 0);

    for (count = 0; count < 12; count++) {
        /* Wait for 0x200 cycles */
        execute_ddr_cmd(dmc, 0x00000200, 0x0001000D, 1);
    }

    execute_ddr_cmd(
        dmc, 0x00000000, ((ddr_info.ranks_to_train << 16) | 0x0000), 0);
    ...
    execute_ddr_cmd(dmc, addr, 0x0001070F, 5);
    execute_ddr_cmd(
        dmc, 0x00000220, ((ddr_info.ranks_to_train << 16) | 0x0301), 0);
    ...
    execute_ddr_cmd(dmc, addr, ((ddr_info.ranks_to_train << 16) | 0x0601), 0);
    delay_ms(1);
    ...
    execute_ddr_cmd(dmc, addr, ((ddr_info.ranks_to_train << 16) | 0x0201), 0);
    execute_ddr_cmd(
        dmc, 0x00000003, ((ddr_info.ranks_to_train << 16) | 0x0101), 0);
    ...
    execute_ddr_cmd(dmc, addr, ((ddr_info.ranks_to_train << 16) | 0x0001), 0);

    addr = 0;
    dimm_spd_t_wtr(&addr, &ddr_info);
    dmc->T_WTR_NEXT = addr;
    ...
    execute_ddr_cmd(
        dmc, 0x00000400, ((ddr_info.ranks_to_train << 16) | 0x0005), 0);
...
static int dmc_bing_pre_init(void)
{
#if defined(PLAT_FVP)
    /* Fill fixed values for FVP */
    ddr_info.speed = 1333;
    ddr_info.number_of_ranks = 1;
    ddr_info.ranks_to_train = 1;
    ddr_info.dimm_mem_width = 8;
    ddr_info.cwl_value = 19;
#else
    int status;
    ...
    if (status != FWK_SUCCESS) {
        FWK_LOG_ERR("[DDR] Error checking DIMM SPD data: %d", status);
        return status;
    }
...
#if !defined(PLAT_FVP)
static int dmc_bing_post_init(void)
{
    int status;
    int i;
    int j;
    ...
    for (i = 0; i < (int)count; i++) {
        ...
        status = dmc_bing_verify_phy_status(id);
        if (status != FWK_SUCCESS) {
            return status;
        }
        FWK_LOG_INFO("[DDR] Done");
...
#if defined(PLAT_FVP)
int dmc_bing_pre_phy_init(struct mod_dmc_bing_reg *dmc)
{
...
int dmc_bing_pre_phy_init(struct mod_dmc_bing_reg *dmc)
{
    ...
    dmc->ADDRESS_MAP_NEXT = 0x00000002;
    dmc->ADDRESS_SHUTTER_31_00_NEXT = 0x11111110;
    dmc->ADDRESS_SHUTTER_63_32_NEXT = 0x11111111;
    dmc->ADDRESS_SHUTTER_95_64_NEXT = 0x11111111;
    dmc->ADDRESS_SHUTTER_127_96_NEXT = 0x11111111;
...
#if !defined(PLAT_FVP)
    status = morello_ddr_phy_config(ddr_id, &ddr_info);
    if (status != FWK_SUCCESS) {
        return status;
    }
#endif
...
{
#if !defined(PLAT_FVP)
    struct mod_dmc_bing_module_config *mod_config =
        (struct mod_dmc_bing_module_config *)config;
