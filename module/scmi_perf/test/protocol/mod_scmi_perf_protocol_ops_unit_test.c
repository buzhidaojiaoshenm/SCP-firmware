...
#include <Mockmod_scmi_perf_protocol_ops_extra.h>
...
void utest_scmi_perf_process_bind_request_protocol_api(void)
{
    int status;
    ...
    api_id = FWK_ID_API(FWK_MODULE_IDX_SCMI_PERF, MOD_SCMI_PERF_PROTOCOL_API);
    ...
    TEST_ASSERT_EQUAL(&scmi_perf_mod_scmi_to_protocol_api, api);
}
...
int scmi_perf_protocol_ops_test_main(void)
{
    UNITY_BEGIN();

    RUN_TEST(utest_scmi_perf_process_bind_request_protocol_api);
...
int main(void)
{
    return scmi_perf_protocol_ops_test_main();
}
#endif
